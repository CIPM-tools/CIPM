...
//	}.expireAfterWrite(2, TimeUnit.MINUTES).resilienceDuration(30, TimeUnit.SECONDS).refreshAhead(false).build();
	...
	private Map<Pair<String, String>, List<ValidationPoint>> resourceUtilCache = new HashMap<>();
	private Map<Pair<String, String>, List<ValidationPoint>> serviceCallCache = new HashMap<>();
...
//		serviceCallCache.clear();
	...
	private List<String> resolveServiceCallTargetIds(InMemoryPCM pcm, String hostId, String serviceId) {
		...
		if (container == null) {
			container = searchResourceContainerInPcm(pcm, hostId);
		}
		if (container != null) {
...
//		}
	...
	private ResourceContainer searchResourceContainerInPcm(InMemoryPCM pcm, String hostId) {
		return pcm
