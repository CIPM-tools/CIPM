...
public final class ResourceCopier {
   ...
   */
  public static Map<Resource, Resource> copyViewSourceResources(final Iterable<Resource> originalResources, final ResourceSet newResourceSet, final Function1<? super EObject, ? extends Boolean> rootElementPredicate) {
   ...
   * @returns A mapping from each original resource to its copy
   */
  public static Map<Resource, Resource> copyViewResources(final Iterable<Resource> originalResources, final ResourceSet newResourceSet) {
    final Function1<EObject, Boolean> _function = new Function1<EObject, Boolean>() {
      public Boolean apply(final EObject it) {
        return true;
      }
    };
    return ResourceCopier.copyResources(originalResources, newResourceSet, true, _function);
  }
  ...
  public static Resource copyViewResource(final Resource originalResource, final ResourceSet newResourceSet) {
    final Map<Resource, Resource> mapping = ResourceCopier.copyViewResources(Collections.<Resource>unmodifiableList(CollectionLiterals.<Resource>newArrayList(originalResource)), newResourceSet);
    return mapping.get(originalResource);
  }

  private static Map<Resource, Resource> copyResources(final Iterable<Resource> originalResources, final ResourceSet newResourceSet, final Boolean copyXmlIds, final Function1<? super EObject, ? extends Boolean> rootElementPredicate) {
    ...
    final Function1<Resource, Boolean> _function = new Function1<Resource, Boolean>() {
      public Boolean apply(final Resource it) {
        return (ResourceCopier.isWritableUmlResource(it));
      }
    };
    List<Resource> _list = IterableExtensions.<Resource>toList(IterableExtensions.<Resource>filter(originalResources, _function));
    ...
    final Map<Resource, Resource> otherMapping = ResourceCopier.copyResourcesInternal(IterableExtensions.<Resource>toList(IterableExtensions.<Resource>filter(originalResources, _function_1)), newResourceSet, copyXmlIds, rootElementPredicate);
   ...
   * element duplication.
   */
  private static Map<Resource, Resource> copyResourcesInternal(final Iterable<Resource> originalResources, final ResourceSet newResourceSet, final Boolean copyXmlIds, final Function1<? super EObject, ? extends Boolean> rootElementPredicate) {
    ...
    for (final Resource originalResource : originalResources) {
      {
        final Function1<EObject, Boolean> _function = new Function1<EObject, Boolean>() {
          public Boolean apply(final EObject it) {
            return (!ResourceCopier.isContainedInOtherThanOwnResource(it, originalResources));
          }
        };
        final List<EObject> elementsContainedInResource = IterableExtensions.<EObject>toList(IterableExtensions.<EObject>filter(originalResource.getContents(), _function));
    ...
    for (final Resource originalResource_1 : originalResources) {
      {
        ...
        final Resource copiedResource = Preconditions.<Resource>checkNotNull(_createResource, "Cannot create resource copy: %s", 
          originalResource_1.getURI());
        final Function1<EObject, Boolean> _function = new Function1<EObject, Boolean>() {
          public Boolean apply(final EObject it) {
            return rootElementPredicate.apply(it);
          }
        };
        final Iterable<EObject> selectedRootElements = IterableExtensions.<EObject>filter(originalResource_1.getContents(), _function);
        final Function1<EObject, EObject> _function_1 = new Function1<EObject, EObject>() {
          public EObject apply(final EObject it) {
            return Preconditions.<EObject>checkNotNull(copier.get(it), "corresponding object for %s is null", it);
          }
        };
        final Iterable<EObject> mappedRootElements = IterableExtensions.<EObject, EObject>map(selectedRootElements, _function_1);
  ...
  private static boolean isContainedInOtherThanOwnResource(final EObject eObject, final Iterable<Resource> resources) {
    final Resource rootContainerResource = EcoreUtil.getRootContainer(eObject).eResource();
    return ((rootContainerResource != eObject.eResource()) && IterableExtensions.contains(resources, rootContainerResource));
  }
    ...
    while ((sourceIterator.hasNext() && targetIterator.hasNext())) {
      {
        ...
        Preconditions.checkState((_eClass == _eClass_1), "non matching elements %s and %s", sourceObject, targetObject);
        target.setID(targetObject, source.getID(sourceObject));
      }
    }
    Preconditions.checkState((!sourceIterator.hasNext()), "source uml resource has too many elements");
    Preconditions.checkState((!targetIterator.hasNext()), "target uml resource has too many elements");
  }
