...
public class ImFacade implements ModelFacade {
	...
	public void saveToDisk() {
		FileBackedModelUtil.synchronize(im, dirLayout.getImFilePath().toFile(), InstrumentationModel.class);
		try {
	...
	public void deactivateAllActionIPs() {
		im.getPoints().forEach(sip -> sip.getActionInstrumentationPoints().forEach(aip -> aip.setActive(false)));
		this.saveToDisk();
	}
	...
	private void loadModel() {
		im = ModelUtil.readFromFile(dirLayout.getImFilePath().toFile(), InstrumentationModel.class);
	}
