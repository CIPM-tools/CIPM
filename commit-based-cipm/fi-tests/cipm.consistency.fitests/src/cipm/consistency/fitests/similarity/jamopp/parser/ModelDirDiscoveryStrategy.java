 ...
 * its contents, similar to depth first search but with a single start point.
 * Uses a given filter to determine whether a given directory is a model source
 * file directory and encompasses one (and only one) model.
 * 
 * TODO Rename methods and parameters
 * 
...
public class ModelDirDiscoveryStrategy implements IModelDirDiscoveryStrategy {
	 ...
	 * 
	 * @param modelDirFilter A filter for determining whether a given directory is a
	 *                       model source file directory and encompasses one (and
	 *                       only one) model
	 */
	...
	/**
	 * @param modelParentDirPath A model source parent directory, which potentially
	 *                           contains model source file directories
	 * @return All model source file directories under the given path to model
	...
	/**
	 * Recursively searches for model source file directories containing Java-Model
	 * files, starting from the given directory, and returns a list of all model
	...
	/**
	 * Recursively searches for model source files directories. All directories
	 * containing models (determined via {@link #isModelSourceDirectory(File)}) will
	 * be added to foundModelDirs, if not already there.
	 * 
	 * @param dirToDiscover  The directory, where the recursive search shall begin
	 ...
	 *           model source file directories are filtered.
	 */
	@Override
